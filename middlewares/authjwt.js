...
isCustomerAndisAdmin = async (req, res,next) => {
    ...
    if (user && user.userType == constants.userType.customer || user.userType == constants.userType.admin) {
        next();
    } else {
        res.status(403).send({
            message: "Requires either Customer or Admin role"
        })
    }
